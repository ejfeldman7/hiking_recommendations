...
if choice == 'Home':
	...
	if visualization_type == 'Frequency Plot/Histogram':
		...
		bin_size = st.slider('Select bin size', min_value=np.floor(df[selected_feature].min()).astype(int), max_value=np.floor(df[selected_feature].max()).astype(int), value=10)  # Adjust the min/max values as needed
